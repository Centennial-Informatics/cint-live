...
<div class="flex flex-col space-y-4 w-full">
	...
	<div class="flex flex-row space-x-4 w-full">
		<button
			class="bg-brand hover:opacity-70 py-2 px-4 font-bold text-brand-lightest rounded-xl text-lg"
			on:click={onClickDivision(STANDARD)}>Download Standard</button
		>
		<button
			class="bg-alt hover:opacity-70 py-2 px-4 font-bold text-alt-lightest rounded-xl text-lg"
			on:click={onClickDivision(ADVANCED)}>Download Advanced</button
		>
	</div>
</div>
